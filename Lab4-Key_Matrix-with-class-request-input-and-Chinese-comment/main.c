...
// input 放輸入的 digit, output 放 輸出的 digit
unsigned int input[4] = {-1, -1, -1, -1}, output[4] = {-1, -1, -1, -1};
// cnt 算輸入底幾個數字, pre_key 記錄上一個 key
...
int getKey(){
	for (col=0; col<4; col++){
		...
		tmp = ((~GPIO_PTA_PADIN) & 0xFF0) >> 4;	// ~: 0 -> 1, 去掉 input 四位
