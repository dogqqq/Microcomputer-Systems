...
unsigned int student_id[9] = {Number_4, Number_0, Number_7, Number_2, Number_6, Number_2, Number_5, Number_1, Number_2};
...
void slide_all_r2l(){
	for (i=0; i<8; i++){
		for(myi = 0; myi < 100; myi++){
			for(k = 0; k < 8; k++){
				if(k > i)	GPIO_PTD_GPIO = 0;
				else	GPIO_PTD_GPIO = student_id[k];
				GPIO_PTA_GPIO = index_7LED[i-k];	// position
				delay1(1000);
			}
		}
	}
...
void slide_all_l2r(){
	for (i=0; i<8; i++){
		for(myi = 0; myi < 100; myi++){
			for(k = 0; k < 8; k++){
				if(k > i)	GPIO_PTD_GPIO = 0;
				else	GPIO_PTD_GPIO = student_id[7-k];
				GPIO_PTA_GPIO = index_7LED[7-i+k];	// position
				delay1(1000);
			}
		}
	}
...
void slide_one_by_one_r2l(){
	for (i=0; i<8; i++){
		for (j=0; j<8-i; j++){
			for(myi = 0; myi < 1000; myi++){
				//move
				GPIO_PTD_GPIO = student_id[i];
				GPIO_PTA_GPIO = index_7LED[j];	// position

				//stay
				delay1(100);
				for(k=0; k<8; k++){
					if(k+1 <= i){
						GPIO_PTD_GPIO = student_id[k];
...
void slide_one_by_one_l2r(){
	for (i=0; i<8; i++){
		for (j=0; j<8-i; j++){
			for(myi = 0; myi < 1000; myi++){
				//move
				GPIO_PTD_GPIO = student_id[7-i];
				GPIO_PTA_GPIO = index_7LED[7-j];	// position

				//stay
				delay1(100);
				for(k=0; k<8; k++){
					if(k+1 <= i){
						GPIO_PTD_GPIO = student_id[7-k];
...
void all(){
	for (i=0; i<8; i++){
		GPIO_PTD_GPIO = student_id[i];
		GPIO_PTA_GPIO = index_7LED[7-i];	// position
		delay1(150);
	}
}
...
int main()
{
	...
	while(1){
		if(dip_check(0) ^ dip_check(1) ^ dip_check(2) ^ dip_check(3)){
			if(dip_check(0) && !flag){
				slide_all_l2r();
				flag = 1;
			}
			else if (dip_check(1) && !flag){
				slide_all_r2l();
				flag = 1;
			}
			else if (dip_check(2) && !flag){
				slide_one_by_one_l2r();
				flag = 1;
			}
			else if (dip_check(3) && !flag){
				slide_one_by_one_r2l();
				flag = 1;
			}
			else {
				all();
